	...
	showOrderCancelOrderButton() {
		return (
			<CustomerBarButton
				...
				image={
					this.props.showView.showNewOrder
						? require('../../images/customer-back-order.png')
						: require('../../images/customer-order.png')
				}
				enabled={
	...
	onOrder = () => {
		if (
			...
			if (!this.props.showView.showNewOrder) {
				...
				if (
					this.props.hasOwnProperty('orderProducts') &&
					this.props.orderProducts.length > 0
				) {
					Alert.alert(
						i18n.t('cancel-order'),
						i18n.t('are-you-sure', {
							doThat: i18n.t('cancel-this-order')
						}),
						[
							{
								text: i18n.t('no'),
