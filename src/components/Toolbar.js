...
class Toolbar extends Component {
			...
			<View style={styles.toolbar}>
				<View style={[styles.leftToolbar]}>
					...
					<TouchableHighlight onPress={() => this.onVersion()}>
						<Text style={[styles.text_style, { marginLeft: 50 }]}>
					...
					<Text
						style={[
							styles.text_style,
							{ marginLeft: 20 },
							this.getNetworkStyle()
						]}>
						{this.getNetworkState()}
					</Text>
					<Text style={[styles.text_style, { marginLeft: 30 }]}>
						{i18n.t('site')}:
					</Text>
					<View style={[styles.site_container, { marginLeft: 20 }]}>
						<Text style={styles.site_text}>
				...
				<View style={[styles.rightToolbar]}>
					<TouchableHighlight onPress={() => this.onSettings()}>
						<Image
							source={require('../images/gear-icon.png')}
							resizeMode="stretch"
							style={[styles.iconSize, { marginRight: 20 }]}
						/>
					</TouchableHighlight>
					{this.getLogoutUI()}
					...
					{this.props.auth.status && (
						<TouchableHighlight
							onPress={() => this.onShowRemoteReport()}>
	...
	getNetworkState = () => {
		return this.props.network.isNWConnected
			? i18n.t('online')
			: i18n.t('offline');
	};
	getNetworkStyle = () => {
